...
def supervisores_datatable(request):
    ...
    puede_crud = puede_administrar_supervisores(request.user)
    ...
    try:
        length = int(
            request.GET.get("length", 25)
        )
    except (TypeError, ValueError):
        length = 25

    length = 25 if length == 0 else min(
        max(length, 10),
        100,
    )
    ...
    for supervisor in queryset:
        ...
        data.append({
            ...
            "regiones": (
                "<br>".join(
                    dict.fromkeys(regiones)
                ) or "-"
            ),
            "niveles": (
                "<br>".join(
                    dict.fromkeys(niveles)
                ) or "-"
            ),
            "situaciones": (
                "<br>".join(
                    dict.fromkeys(situaciones)
                ) or "-"
            ),
