...
from .domain.catalogs import activity_catalogs, expandir_rangos
...
def filtrar_datos_actividad(request):
    try:
        ceic, grados, secciones = activity_catalogs(integer_param(request, "modalidad"), integer_param(request, "nivel"))
        return JsonResponse({"ceic": list(ceic.values("c_ceic", "descripcion")), "grado": list(grados.values("c_grado_anio", "nombre_grado_anio")), "secciones": list(secciones.values("c_seccion", "nombre_seccion"))})
    except ValidationError as exc:
