...
def filter_data_efec_aban_rep(request):
    ...
    try:
        ...
        query_efec="""
            ...
            FROM indicadores.tasas1_primaria_ra2024 as ts
        ...
        titulo = f'<b>Tasa Efectiva, Abandono y Repitencia del Nivel Primario por Año</b> - Regional: {cueanexo}'
        fig = go.Figure()
        ...
        return render(request, 'indicadores/efecabanrepprimariareg.html', context)
    finally:
        cursor.close()
        connection.close()
...
def filter_data_efec_aban_rep_total(request):
    ...
    try:
        ...
        titulo = f'<b>Tasa Efectiva, Abandono y Repitencia del Nivel Primario por Año</b> : {cueanexo}'
        fig = go.Figure()
        fig.add_trace(go.Bar(x=labels, y=tasa_efectiva, name='Efectiva', marker_color='red'))
        fig.add_trace(go.Bar(x=labels, y=tasa_abandono, name='Abandono', marker_color='green'))
        fig.add_trace(go.Bar(x=labels, y=tasa_repitencia, name='Repitencia', marker_color='blue'))
        ...
        return render(request, 'indicadores/efecabanrepprimariatotal.html', context)
    finally:
        cursor.close()
        connection.close()
