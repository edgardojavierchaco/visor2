...
from .models import EventoAuditoria, TipoPersonal


def filtered_activities(request):
    qs = activity_scope(request.user)
    for param, field in (("cueanexo", "cueanexo"), ("tipo_personal", "tipo_personal_id"), ("estado", "estado"), ("validacion", "validacion")):
...
class PersonasListView(View):
    def get(self, request):
        ...
        filters = any(request.GET.get(key) for key in ("cueanexo", "tipo_personal", "estado", "validacion", "q"))
        if filters:
        ...
        totals = activities.aggregate(cargos=Count("pk"), docentes=Count("pk", filter=Q(tipo_personal_id=1)), no_docentes=Count("pk", filter=Q(tipo_personal_id=2)), pendientes=Count("pk", filter=Q(validacion="BORRADOR")))
        return render(request, "bnh/personas/list.html", {"page_obj": page, "personas": page.object_list, "total_personas": page.paginator.count, "totals": totals, "query": params.urlencode(), "instituciones": scoped_offers(request.user).order_by("cueanexo_str").values("cueanexo_str", "nom_est").distinct(), "tipos_personal": TipoPersonal.objects.order_by("c_tpersonal"), "filters": request.GET})
...
class PersonaDetailView(View):
    def get(self, request, pk):
        ...
        activities = activity_scope(request.user, include_deleted=True).filter(persona=person).select_related("tipo_personal", "cond_actividad", "ceic", "sit_revista", "modalidad", "niveles", "modalidad_curricular", "nivel_curricular", "espacio_curricular", "grado_anio", "secciones").order_by("eliminado", "cueanexo", "-f_desde", "pk")
