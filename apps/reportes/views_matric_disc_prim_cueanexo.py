...
def filtrado_matric_disc_prim_cueanexo(request):
    ...
    return render(request, 'reportes/filter_matric_disc_prim_cueanexo.html')
    ...
    if request.method == 'POST':     
        ...
        for row in rows:
            datamatriccueanexo.append({
                ...
                'grado_anio':row[2],                
                'ceguera': int(row[3]),
                'dism_visual': int(row[4]),
                'sordera': int(row[5]),
                'hipoacusia': int(row[6]),
                'intelectual': int(row[7]),
                'motora_pura': int(row[8]),
                'neuromotora': int(row[9]),
                'tea': int(row[10]),
                'region_loc': row[16],
                'localidad': row[18],
            })
        ...
        return render(request, 'reportes/matric_disc_prim_cueanexo.html', {'datamatriccueanexo': datamatriccueanexo, 'nvistamatricula': nvistamatricula, 'nrelevamiento':'RELEVAMIENTO 2024'})
