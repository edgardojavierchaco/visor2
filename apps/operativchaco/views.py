...
from .forms import ExamenLenguaAlumnoForm
...
@login_required
def cargar_examen_lengua(request):
    ...
    if request.method == 'POST':
        form = ExamenLenguaAlumnoForm(request.POST, user=request.user, region=region)
        if form.is_valid():
            ...
            form.save()
            return redirect('operativ:examen_lengua_listado')  
    else:
        form = ExamenLenguaAlumnoForm(user=request.user, region=region)

    return render(request, 'operativchaco/lengua/examen_lengua_form.html', {'form': form})
