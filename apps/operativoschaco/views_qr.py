...
from .models import ExamenAlumnoCueanexoL, CierreCargaL

def cerrar_carga(request):
    user = request.user  # Objeto User
    registros = ExamenAlumnoCueanexoL.objects.filter(alumno__cueanexo=user.username).count()
    fecha_cierre = now()

    # Guardar el cierre en la base de datos
    cierre = CierreCargaL.objects.create(usuario=user, total_registros=registros)
    ...
    return FileResponse(pdf_buffer, as_attachment=True, filename='cierre_carga_lengua.pdf')
