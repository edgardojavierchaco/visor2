...
class CapaUnicaOfertas(models.Model):
    ...
    def __str__(self):
        return self.cueanexo or f"Sin CUE ({self.nom_est})"
    

class Colectivoss(models.Model):
    codigo = models.CharField(max_length=50, primary_key=True)
    direccion = models.TextField()
    long = models.FloatField()
    lat = models.FloatField()
    cod = models.CharField(max_length=10)
    distan_paradas = models.IntegerField(null=True, blank=True)
    ...
    def __str__(self):
        return f"{self.lineas} - {self.direccion}"
