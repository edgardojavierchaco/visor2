...
def safe_cache_key(data):
    ...
    return "cueradio_" + hashlib.md5(raw.encode()).hexdigest()

# ------------------------------
# Vista template
# ------------------------------
def cueradio_view(request):
    return render(request, "mapa/cueradio.html")
...
def api_filtros(request):
    with connection.cursor() as cursor:
        cursor.execute("""
            SELECT DISTINCT oferta, sector, ambito
            FROM v_capa_unica_ofertas_ant
        """)
        rows = cursor.fetchall()

    ofertas = sorted(set(r[0] for r in rows if r[0]))
    sectores = sorted(set(r[1] for r in rows if r[1]))
    ambitos = sorted(set(r[2] for r in rows if r[2]))
...
# ------------------------------
@require_GET
def api_cueradio(request):
    cue = request.GET.get('cueanexo')
    radio = int(request.GET.get('radio', 1000))
    oferta = request.GET.get('oferta')
    sector = request.GET.get('sector')
    ambito = request.GET.get('ambito')

    key = safe_cache_key(request.GET.dict())
    cached = cache.get(key)
    if cached:
        return JsonResponse(cached)

    data = []
    kpis = {'total': 0, 'sector': {}, 'oferta': {}}
    ...
    with connection.cursor() as cursor:
        ...
        if not center:
            return JsonResponse({'data': [], 'kpis': kpis})

        lat, lng = center

        query = """
            SELECT cueanexo, lat, long, nom_est, oferta, sector, ambito
            FROM v_capa_unica_ofertas_ant
            WHERE ST_DWithin(
    ...
    for r in rows:
        c, lat, lng, nom, ofe, sec, amb = r
        color = "red" if c == cue else "green"
        data.append({
            'cue': c,
            'lat': lat,
            'lng': lng,
        ...
        kpis['total'] += 1
        kpis['sector'][sec] = kpis['sector'].get(sec, 0) + 1
        kpis['oferta'][ofe] = kpis['oferta'].get(ofe, 0) + 1

    result = {'data': data, 'kpis': kpis}
