...
def obtener_cueanexos(username):

    cuil = formatear_cuil(username)
    ...
    with connection.cursor() as cursor:
        cursor.execute("""
            ...
            WHERE resploc_cuitcuil = %s
            AND TRIM(oferta) = 'Común - Primario de 7 años'
        """, [cuil])
...
def calcular_estadisticas_por_cueanexo(username):
    ...
    # cueanexos = obtener_cueanexos(username)
    ...
    with connection.cursor() as cursor:
        ...
        # =====================
        cursor.execute("""
            SELECT COUNT(dni_alumno)
            FROM cenpe."Evaluacion_Lectora"
            WHERE cueanexo = ANY(%s)
        """, [cueanexos])
        total_dni_sin_desempenio = cursor.fetchone()[0] or 0
        ...
        cursor.execute("""
            ...
            WHERE cueanexo = ANY(%s)
              AND asistencia = 'true'
        """, [cueanexos])
        total_dni_presentes = cursor.fetchone()[0] or 0
        ...
        """, [cueanexos])
        total_dni_ausentes = cursor.fetchone()[0] or 0
    ...
    return (
        resultados_total_dni,
        total_dni_sin_desempenio,
        (promedios[0] or 0,),
        (promedios[1] or 0,),
        (promedios[2] or 0,),
        (promedios[3] or 0,),
...
def tu_vista(request):
    ...
    cueanexos = CapaUnicaOfertas.objects.filter(resploc_cuitcuil=cuil_con_caracter, oferta='Común - Primaria de 7 años ').only('cueanexo')
    #username = request.user.username
    lista_cueanexos_validos=[]
    for i in cueanexos:
        lista_cueanexos_validos.append(f'{i.cueanexo}')
    resultados_total_dni, total_dni_sin_desempenio, resultado_promedio_velocidad, resultado_promedio_precision, resultado_promedio_prosodia, resultado_promedio_comprension, total_dni_presentes, total_dni_ausentes = calcular_estadisticas_por_cueanexo(lista_cueanexos_validos)
    ...
    for i, atributo in enumerate(atributos):
        ...
        if total_dni_presentes == 0:
            etiquetas = [f"{label}: {conteo} (0%)" for label, conteo in zip(labels, valores)]
        else:
            etiquetas = [
                f"{label}: {conteo} ({(conteo / total_dni_presentes * 100):.2f}%)"
                for label, conteo in zip(labels, valores)
            ]
...
def mostrar_grafico_reg(request):
    ...
    with psycopg2.connect(
        ...
        with conn.cursor() as cursor:
            ...
            # Lista para parámetros de la consulta
            parameters1 = []
            
            if mostrar_todo:
                # Si mostrar_todo es True, no se agrega filtro de region
                if ambitos_seleccionados:
                    query1 += " AND ambito = %s"
                ...
                # Si no es "mostrar todo", filtrar por región también
                if regiones_seleccionadas:
                    query1 += " AND region = %s"
            ...
            if mostrar_todo:
                # Si mostrar_todo es True, no se agrega filtro de región
                if ambitos_seleccionados:
                    query1 += " AND ambito = %s"
                    parameters1.append(ambitos_seleccionados)
                if sectores_seleccionados:
            ...
            # Definir los filtros de acuerdo con los valores seleccionados
            if mostrar_todo:
                # Si mostrar_todo es True, no se filtra por región, solo por ámbito y sector
                if ambitos_seleccionados:
                    query1 += " AND ambito = %s"
                    parameters1.append(ambitos_seleccionados)
                if sectores_seleccionados:
            ...
            # Agregar filtros dependiendo de la opción seleccionada
            if mostrar_todo:
                # Si mostrar_todo es True, no se filtra por región, solo por ámbito y sector
                if ambitos_seleccionados:
                    query1 += " AND ambito = %s"
