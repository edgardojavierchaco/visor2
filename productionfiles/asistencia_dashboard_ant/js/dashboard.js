...
async function actualizarDashboard() {
    ...
    try {
        ...
        const [
            ...
            fetchJSON(`${CFG.urls.ranking}?${params}`),
            fetchJSON(`${CFG.urls.alertas}?${params}`)
        ]);
...
function pintarCalidad(resultado) {
    ...
    if (!datos.length) {
        tbody.innerHTML = `<tr><td colspan="10" class="text-center py-5 text-muted">${escapeHtml(resultado.message || "Sin datos de calidad procesados para el período seleccionado.")}</td></tr>`;
        return;
    }
    datos.forEach(fila => {
        tbody.insertAdjacentHTML("beforeend", `
            <tr>
                ...
                <td class="text-center">${formatNumero(fila.dias_excepcion)}</td>
                <td class="text-center">${formatNumero(fila.dias_evento_excluyente)}</td>
                <td class="text-center">${formatNumero(fila.dias_esperados)}</td>
                <td class="text-center">${formatNumero(fila.dias_registrados)}</td>
                <td class="text-center"><strong>${formatNumero(fila.dias_sin_registro)}</strong></td>
